...
- **全站搜索**：中文、English、别名、标签、正文全文匹配，图鉴元素、UI 风格与 UI 库一并命中；`/` 聚焦，`↑↓` 选择，`Enter` 跳转
- **前端元素图鉴**：网页与 App 常见界面元素按「基础 / 表单 / 导航 / 数据展示 / 反馈 / 浮层 / 媒体 / 布局 / 视觉技法 / 动效」分组，每个元素 = 标准名与别名 + 一句话描述 + 真实呈现标本；搜「走马灯」直达轮播、搜「毛玻璃」直达毛玻璃，词条页与图鉴双向互链
- **UI 风格图鉴**：拟物 / 扁平 / Material / 玻璃拟态 / 瑞士 / Bento Grid / 酸性设计等主流视觉风格，「同一套组件、多种风格渲染」对比式小样，每个风格配特征清单、代码要点与「怎么对 AI 说」句式——风格这样描述，AI 才能一次还原
- **UI 库图鉴**：shadcn/ui、MUI、Ant Design、Tailwind 等主流组件库与样式方案，按生态 / 场景 / 收费 / 标签筛选候选，每库用「风格小样」呈现其设计语言，并配「怎么向 AI 声明技术栈」的句式与误述——选对库、说对话，不返工
- **学习路线**：循序渐进的路径把散落词条串起来，含 UI 沟通速查线
- **学习进度与收藏**：localStorage 本地记录「已掌握」，分类进度条可视化
- **访问统计**：自托管轻量计数（复用反馈服务进程，无第三方依赖），页脚展示总访问与今日访问
...
│   ├── js/
│   │   ├── data/           # 十大分类词条数据 + 图鉴元素/风格/库数据（内容都在这）
│   │   ├── visuals.js      # SVG 图解注册表
│   │   ├── visual-demos.js # 图鉴元素呈现效果注册表
│   │   ├── ui-style-demos.js # UI 风格小样注册表（同一组件套件 × 多风格皮肤）
│   │   ├── anims.js        # 动画注册表
│   │   └── …               # 数据聚合 / 搜索 / 组件 / 视图 / 路由 / 入口
│   └── img/                # 图片媒体槽位
├── tools/
